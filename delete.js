...
let teamEx = ["Bob", "Henry", "George", "Ico", "Mooky", "Hendo", "Mark", "Jeter", "Vlad", "Marco",
              ...
              "Galileo", "Gael", "Tyrian", "Mac", "Maddax", "Mackie", "Magnus", "Magic", "Nas", "Narcisus",
...
let teamWhy = ["Johnson", "Smart", "McDreamy", "Bell", "Bird", "Jordan", "Rosario", "Money", "Lexington", "Wood",
...
const playerNames = () => {
    ...
    let random = Math.floor(Math.random() * teamEx.length)
    let random2 = Math.floor(Math.random() * teamWhy.length)
...
const runners = (sc) => {
    ...
    if (runOne == 2) {
        sc.players.position2.runs += 1
        runOne = 0
        runsIn += 1
    } else if (runOne == 3) {
        sc.players.position3.runs += 1
        runOne = 0
        runsIn += 1
    } else if (runOne == 4) {
        sc.players.position4.runs += 1
        runOne = 0
        runsIn += 1
    } else if (runOne == 5) {
        sc.players.position5.runs += 1
        runOne = 0
        runsIn += 1
    } else if (runOne == 6) {
        sc.players.position6.runs += 1
        runOne = 0
        runsIn += 1
    } else if (runOne == 7) {
        sc.players.position7.runs += 1
        runOne = 0
        runsIn += 1
    } else if (runOne == 8) {
        sc.players.position8.runs += 1
        runOne = 0
        runsIn += 1
    } else if (runOne == 9) {
        sc.players.position9.runs += 1
        runOne = 0
        runsIn += 1
    }
...
const homer = (te) => {
    ...
    if (firstBase >= 1) {
        if (firstBase == 2) {
            te.players.position2.runs += 1
            runsIn += 1
            firstBase = 0
        } else if (firstBase == 3) {
            te.players.position3.runs += 1
            runsIn += 1
            firstBase = 0
        } else if (firstBase == 4) {
            te.players.position4.runs += 1
            runsIn += 1
            firstBase = 0
        } else if (firstBase == 5) {
            te.players.position5.runs += 1
            runsIn += 1
            firstBase = 0
        } else if (firstBase == 6) {
            te.players.position6.runs += 1
            runsIn += 1
            firstBase = 0
        } else if (firstBase == 7) {
            te.players.position7.runs += 1
            runsIn += 1
            firstBase = 0
        } else if (firstBase == 8) {
            te.players.position8.runs += 1
            runsIn += 1
            firstBase = 0
        } else if (firstBase == 9) {
            te.players.position9.runs += 1
            runsIn += 1
            firstBase = 0
        }
    }
...
function calcBatterAttributeBonus() {
    let startNumber = 2
    let resultArray = []
    if (disparity >= 0) {
        for (let i = 0; i <= disparity; i++) {
            startNumber > 0 ? startNumber -= .1 : undefined 
        }
    } else {
        for (let i = 0; i >= disparity; i--) {
            startNumber >= 2 ? startNumber += .1 : undefined 
        }
    }
    resultArray = [startNumber, startNumber * 2, startNumber / 5]
    return resultArray
}

function calcPitcherAttributeBonus() {
    let startNumber = 2
    let resultArray = []
    if (disparity >= 0) {
        for (let i = 0; i <= disparity; i++) {
            startNumber > 0 ? startNumber -= 4 : undefined 
        }
    } else {
        for (let i = 0; i >= disparity; i--) {
            startNumber >= 2 ? startNumber += 4 : undefined 
        }
    }
    resultArray = [startNumber, startNumber / 5, startNumber / 10]
    return resultArray
}


const pitchers = () => {
    console.log(`${team1.name}: ${Math.floor(team1.players.position1.attributes[0])}, 
                 ${team2.name}: ${Math.floor(team2.players.position1.attributes[0])}, 
                 ${team3.name}: ${Math.floor(team3.players.position1.attributes[0])}, 
                 ${team4.name}: ${Math.floor(team4.players.position1.attributes[0])}, 
                 ${team5.name}: ${Math.floor(team5.players.position1.attributes[0])}, 
                 ${team6.name}: ${Math.floor(team6.players.position1.attributes[0])}, 
                 ${team7.name}: ${Math.floor(team7.players.position1.attributes[0])}, 
                 ${team8.name}: ${Math.floor(team8.players.position1.attributes[0])}, 
                 ${team9.name}: ${Math.floor(team9.players.position1.attributes[0])}, 
                 ${team10.name}: ${Math.floor(team10.players.position1.attributes[0])}, 
                 ${team11.name}: ${Math.floor(team11.players.position1.attributes[0])}, 
                 ${team12.name}: ${Math.floor(team12.players.position1.attributes[0])}, 
                 ${team13.name}: ${Math.floor(team13.players.position1.attributes[0])}, 
                 ${team14.name}: ${Math.floor(team14.players.position1.attributes[0])}, 
                 ${team15.name}: ${Math.floor(team15.players.position1.attributes[0])}, 
                 ${team16.name}: ${Math.floor(team16.players.position1.attributes[0])}, 
                 ${team17.name}: ${Math.floor(team17.players.position1.attributes[0])}, 
                 ${team18.name}: ${Math.floor(team18.players.position1.attributes[0])}, 
                 ${team20.name}: ${Math.floor(team20.players.position1.attributes[0])}, 
                 ${team21.name}: ${Math.floor(team21.players.position1.attributes[0])}, 
                 ${team22.name}: ${Math.floor(team22.players.position1.attributes[0])}, 
                 ${team23.name}: ${Math.floor(team23.players.position1.attributes[0])}, 
                 ${team24.name}: ${Math.floor(team24.players.position1.attributes[0])}, 
                 ${team25.name}: ${Math.floor(team25.players.position1.attributes[0])}, 
                 ${team26.name}: ${Math.floor(team26.players.position1.attributes[0])}, 
                 ${team27.name}: ${Math.floor(team27.players.position1.attributes[0])}, 
                 ${team28.name}: ${Math.floor(team28.players.position1.attributes[0])}, 
                 ${team29.name}: ${Math.floor(team29.players.position1.attributes[0])}, 
                 ${team30.name}: ${Math.floor(team30.players.position1.attributes[0])}
    ...
    for (let i = 1; ; i++) { // innings for loop (9 innings)
        ...
        if (i % 2 == 1) { // this would be an odd inning like top of the first
            ...
            for (let j = 1; outs < 3; j++) { // run through team1s batting lineup
                ...
                atBat(a.players[`position${j+1}`], b.players, a.players)       
            }
            ...
            for (let j = 1; outs < 3; j++) { // run through team1s batting lineup
                ...
                atBat(b.players[`position${j+1}`], a.players, b.players) 
            }
        }   
...
function atBat(a, b, otherA) {  /// atBat(a.players[`position${j+1}`], b.players) 
            ...
            if (a.attributes[0] + disparity >= random) { // ground ball passes the infield
                ...
                if (firstBase == 3 && secondBase == 0) { // the pitcher and the player at bat can't steal, so only 7 player variations
                    ...
                    if (otherA.position3.attributes[3] + disparity >= random) {
                        ...
                        j -= 1 // hopefully this resets the at bat
